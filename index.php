...
//para el nombre 
$conexion = conexion();
$consulta_datos = ("SELECT nombre FROM clientes");
$datos = $conexion->query($consulta_datos);
$datos = $datos->fetchAll();

//para el medidor
$consulta_medidor = ("SELECT  medidor FROM clientes");
$datos_medidor = $conexion->query($consulta_medidor);
$datos_medidor = $datos_medidor->fetchAll();

//poligono 
$consulta_poligono = ("SELECT  lote_poligono FROM clientes");
$datos_poligono = $conexion->query($consulta_poligono);
$datos_poligono = $datos_poligono->fetchAll();


?>

<body> <?php include 'inc/navbar.php'; ?> <div class="container bg-white p-4 rounded card">
        ...
        <form id="aguaForm" action="php/recibo.php" method="post" autocomplete="off">
            <div class="row">
                <div class="col-md-6 border-end">
                    ...
                    <div class="mb-3"> <label class="form-label">NOMBRE DEL USUARIO</label> <select id="nombre_usuario" class="form-select">
...
<!--N° MEDIDOR-->
                    <div class="mb-3"> <label class="form-label">N° MEDIDOR</label> <select id="medidor_usuario" class="form-select">
                          <?php   foreach ($datos_medidor as $dato) {
                                echo "<option data-medidor='" . $dato['medidor'] . "'>" . $dato['medidor'] . "</option>";
                            }
                            ?></option>
                        </select> </div>
...
<!--poligono-->
                    <div class="mb-3"> <label class="form-label">LOTE Y POLIGONO</label> <select id="poligono_usuario" class="form-select">
                            <?php   foreach ($datos_poligono as $dato) {
                                echo "<option>" . $dato['lote_poligono'] . "</option>";
                            }
                            ?></option>
                        </select> </div>
